...
- 初始化UI界面
   ...
   - 构建界面上需要的各类控件，通过继承基类并手动将设计师界面上的控件提升为对应类型，使控件设计个性化，并能够提供自定义的函数如`void Map_ui::paint(QPainter &,int width, int height);`

   - 获取创建游戏需要的数据：`游戏时间-游戏人数`，通过`Dialog`将相应值捕获，最后通过`command`指导底层模块创建数据实体；

   - 通过共享指针绑定ui界面已经创建的控件。

- 处理界面反馈逻辑

   界面反馈逻辑包括键盘事件、绘图事件以及计时器事件，这些事件既分离又有机的结合在一起：

   - 计时器事件是游戏的重要一环，键盘事件的处理基于计时器的回调函数实现。在QT中使用`信号与槽` 机制，用`connect`连接计时器变量的回调信号与每个时间周期的`move()`函数（即槽函数）；在这个槽函数中又根据我们游戏的需要设计了倒计时模块与玩家实时领地数模块；
   - 键盘事件会导致底层数据改变，出发绘图事件。绘图事件是我们的最终目标，而键盘事件是导致绘图事件需要回调的根本原因，又因为绘图的刷新率与时钟周期有关，因此键盘事件的一部分逻辑——玩家控制方向处理——放在了计时器的槽函数中，用`command->exec()`的形式接入Model&View Model层的功能；另一部分键盘逻辑和游戏状态有关，通过`游戏状态信号-状态处理槽函数`形式处理；
   - 绘图事件专注于每次时钟周期需要重新绘制的数据，包括玩家的实时领地数。

- `Command`绑定与绘图数据获取

   - 由于`View层`与`Model层`的解耦，`View层`可以专注于视图设计，而完全不用考虑实现方法，用`command->exc()`的形式接入`View_Model&Model`的功能；
   - 如果需要访问Model层的数据，在类内定义对应对应的仿函数。如`std::function<QList<QList<Block>>(void)> get_blocks_colors  `是获取每个`Block`染色信息的仿函数，在`App`层再利用`lambda`表达式，绑定到`Model&View Model`层提供的访问接口上。

##### 4.1.5 App

- 将`View`层的共享指针、仿函数绑定到`Model&View Model`上；
- 将`View Model`的命令绑定到`View`上。
